...
internal sealed class ChatSession
{
  ...
  {
    _cfg = cfg; _ui = ui; _rt = rt; _history = new ChatHistory();
  ...
  public void Run()
  {
    ...
    while (true)
    {
      ...
      if (input.Equals(_cfg.ExitCommand, StringComparison.OrdinalIgnoreCase)) break;
      if (input.Equals(_cfg.HelpCommand, StringComparison.OrdinalIgnoreCase)) { _ui.PrintHelp(_cfg); continue; }
      if (input.Equals(_cfg.ResetCommand, StringComparison.OrdinalIgnoreCase)) { _history.Clear(); _history.AddSystem(_cfg.SystemPrompt); _ui.Info("History cleared."); continue; }
  ...
  private string GenerateOneTurnStreaming(string prompt)
  {
    ...
    // Helpful for DML
    try { gp.SetSearchOption("past_present_share_buffer", true); } catch { }
  ...
  private static int FindRoleToken(string s)
  {
    ...
    int cut = -1;
    if (a >= 0) cut = a; if (b >= 0) cut = (cut < 0) ? b : Math.Min(cut, b); if (c >= 0) cut = (cut < 0) ? c : Math.Min(cut, c);
    return cut;
  }
  private static string DiffSuffix(string prev, string now)
  {
    ...
    int i = 0; while (i < n && prev[i] == now[i]) i++; return now.Substring(i);
  }
  private static string CleanPhi3Artifacts(string s)
  {
    if (string.IsNullOrWhiteSpace(s)) return "";
    s = s.Replace("- [response]:", "", StringComparison.OrdinalIgnoreCase).Trim();
    if (s.StartsWith("[response]:", StringComparison.OrdinalIgnoreCase)) s = s.Substring("[response]:".Length).Trim();
    if (s.StartsWith("response:", StringComparison.OrdinalIgnoreCase)) s = s.Substring("response:".Length).Trim();
    int cut = FindAnyRoleToken(s); if (cut >= 0) s = s.Substring(0, cut).Trim();
    return s;
  }
  private static int FindAnyRoleToken(string s)
  {
    ...
    int b = s.IndexOf("<user>", StringComparison.Ordinal);
    int c = s.IndexOf("<assistant>", StringComparison.Ordinal);
    int cut = -1; if (a >= 0) cut = a; if (b >= 0) cut = (cut < 0) ? b : Math.Min(cut, b); if (c >= 0) cut = (cut < 0) ? c : Math.Min(cut, c); return cut;
  }
}
